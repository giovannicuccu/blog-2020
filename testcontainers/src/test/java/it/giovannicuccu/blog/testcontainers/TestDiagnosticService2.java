...
public class TestDiagnosticService2 {
	...
	public void testFileBeatKafka() throws Exception {
		for (int i=0; i<10;i++) {
			loggerFileBeat.debug("|name={}|type={}|status={}|start_ts={}|ela={}|","name"+i,"HTTP_SERVICE", "OK",formatter.format(LocalDateTime.now()),i+100);
		}
		...
		try (
				...
				GenericContainer filebeatContainer=new GenericContainer<>(
						...
						.withDockerfile(Paths.get("src/test/resources/filebeat/Dockerfile")))
						.withNetwork(network)
						.withCopyFileToContainer(MountableFile.forClasspathResource("filebeat/diagnostic.log"),"/var/log/app/diagnostic.log");
		) {
			...
			DiagnosticService diagnosticService= new DiagnosticService();
		     ...
		     while (i<tries) {
		         ...
		         for (ConsumerRecord<String, String> record : records) {
		        	 ...
		        	 Map<String,Object> recordAsMap=objectMapper.readValue(record.value(),new TypeReference<HashMap<String,Object>>() {});
		     ...
		     consumer.close();
		     Assert.assertEquals(10,diagnosticService.loadAllEvents().size());
		}
	}
